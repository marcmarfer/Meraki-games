...
const WordsBox1 = () => {
  ...
  const [selectedLevel, setSelectedLevel] = useState("easy"); // Nivel por defecto
  ...
  const onDrop = (e, target) => {
    ...
    if (target === 'box-a') {
      ...
      if (selectedLevel === "easy" && content.startsWith('a')) {
        setScore(score + 1); // Incrementar el puntaje si la palabra empieza con 'a'
      } else if (selectedLevel === "medium" && content.startsWith('a') && content.endsWith('s')) {
        setScore(score + 1); // Incrementar el puntaje si la palabra empieza con 'a' y termina con 's'
      } else if (selectedLevel === "hard" && content.startsWith('c') && content.includes('a')) {
        setScore(score + 1); // Incrementar el puntaje si la palabra empieza con 'c' y contiene 'a'
      } else {
        setMistakes(mistakes + 1); // Incrementar los errores si no cumple con las condiciones
      }
    }
  };
